...
```
### Кортежи (tuple)
**Картеж** - по сути тоже самое, что и список, но это некая константа, в которую мы добавляем элементы и изменить уже картеж нельзя, и второе отличие - картеж занимает меньше памяти чем список. Картеж объявляется как:
```
